...
According to Kondo, cleaning and tidying your home are two very different things. Tidying is a large undertaking that should only have to do once. Cleaning is the process of putting things back in their place, and putting your home back into tidy order. Cleaning becomes much easier when your home is already tidy. Cleaning is very arduous in a non-tidy home. 

Cleaning and tidying data are also very different things. Cleaning data involves reformatting character variables that are supposed numeric, or removing columns that are all NAs. Cleaning data could also include correcting mispellings of variable values, like "oragne" instead of "orange", or making things all lower or upper case. 
...
In the home, untidyness can be caused by many different things. Perhaps you never put anything away. Or maybe you hold onto things way longer than you should. If could be that you keep buying things that have no use to you, but seemed exciting at the time of purchase. I was really a combination of all three of these things before I started "Kondo-ing" my life. 
...
This workshop was written in the `learnr` format, so you can code on the website! Everytime you see a code box, you can write and run your own code! Play around!  
...
This rule from Mari Kondo is totally and utterly the *opposite* for tidying your data.  
...
You may have the same object type stored in multiple data frames. 
Determine the types of units in your data first that may be stored across several data frames or data objects. 
...
ranks2 <- ranks2 %>% filter(!is.na(rank))
dim(ranks2)
summary(ranks2)
```
...
```{r lines-ranks, exercise=TRUE, exercise.lines = 15}
ggplot(data = ranks2, aes(x = week, y = rank, group = songID)) + 
  geom_line(alpha = .5) + 
  scale_y_reverse()
```
...
```{r days-weeks, exercise=TRUE, exercise.lines = 15}
ranks2 %>% mutate(date.actual = date.entered + (week-1)*7) %>% 
...
```{r only-ones, exercise=TRUE, exercise.lines = 15, fig.width=8}
ranks2 %>% mutate(date.actual = date.entered + (week-1)*7) %>% 
...
Similar to the billboard data: it's wide when it should be long. Use the box below to ti
